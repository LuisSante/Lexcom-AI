...
import { NavbarItemsPricing } from "../logic/components_dashboard/NavbarItemsPricing";
import Login from "../components_home/Login";

const NavbarPricing: React.FC<NavbarProps> = ({ isScrolling }) => {
    ...
    return (
        <nav className={`navbar ${isScrolling > 10 ? 'scrolling' : ''}`}>
            ...
            <ul className={(clicked && isScrolling > 10) ? "nav_menu active scrolling_bars_container active" :
                ...
                {NavbarItemsPricing.map((item, index) => (
